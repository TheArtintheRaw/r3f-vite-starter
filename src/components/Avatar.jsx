...
export function Avatar({
  ...
  bottomColor = 'brown',
  id,
  avatarUrl = 'https://models.readyplayer.me/64fcff14a9fb106b21ad49ae.glb',
  ...props
}) {
  ...
  const { actions } = useAnimations([walkAnimation[0], idleAnimation[0], danceAnimation[0]], avatarRef)
  ...
  useFrame((state) => {
    ...
    if (path?.length && group.current.position.distanceTo(path[0]) > 0.1) {
      const direction = group.current.position.clone().sub(path[0]).normalize().multiplyScalar(MOVEMENT_SPEED)
    ...
    if (id === user) {
      ...
      state.camera.position.y = group.current.position.y + 20
